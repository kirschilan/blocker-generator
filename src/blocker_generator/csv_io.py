...
def write_jira_csv(rows: List[IssueRow], path: Path) -> None:
    ...
    'Sprint' (Issue #7) and 'Labels' (PBI 2.0c) headers mean duplicate
    column names, which a dict-keyed row can't represent."""
    path.parent.mkdir(parents=True, exist_ok=True)
    label_slots = max_label_span(rows)
    sprint_slots = max_sprint_span(rows)
    with path.open("w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(build_header(label_slots, sprint_slots))
        for row in rows:
            writer.writerow(row_to_csv_row(row, label_slots, sprint_slots))
