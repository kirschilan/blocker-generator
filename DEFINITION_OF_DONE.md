...
- [ ] Produces a measurable change in the running software's actual output (CLI/data), **or** explicitly states the exception and how "unchanged" was verified (e.g., "refactor — output reconfirmed byte-identical") — Governance Rule 9
...
- **Check for a designated branch before creating one** (added 2026-08-24): if the session's task instructions name a specific branch, use it — don't invent a new name. Caught this iteration when Code pushed to a self-invented branch instead of the designated one and had to reconcile it afterward.
- **Branch-deletion list at end of each PBI** (added 2026-08-24, PO's standing request): Code hands the PO the list of branches now safe to delete (merged, no open PR) at the end of work on each PBI, not just at end-of-session or end-of-Iteration. Code has no delete-branch permission in this environment (git push --delete and the GitHub API both refuse it) — deletion is always the PO's action; Code's job is to make the list current and correct on this cadence.
...
- **A PBI cannot be Done while its branch is unmerged (added 2026-08-25).** This project has no condition that legitimately separates "Done" from "merged": Artifact 5's checklist already requires Product Owner sign-off before Done, and in this trunk-based, single-day-Iteration, few-participant flow, sign-off and merge are the same event — there's no external review queue, staged-rollout gate, or compliance hold in this project's actual operating model that would justify a PBI being Done while parked on an open branch. If such a gate is ever introduced, this rule needs revisiting then, not before. Until it is, "Done" implies "merged," full stop — closing an Iteration with real work still sitting unmerged (as happened in Iteration 4, caught only when the PO asked "anything undone?") is the rule failing to enforce itself, not a legitimate exception.

### 8. Vendor Format Verification

- **Rule:** Any CSV column that claims to mirror a real vendor export (Jira, Xray, or any future vendor) must be verified against that vendor's actual documented export behavior — a cited source (vendor docs, a known issue-tracker ticket like JRACLOUD-xxxxx) — before it's locked into `ARCHITECTURE.md`. "It's probably like X" is not verification. A column that's a deliberate internal design choice (not vendor-native) must say so explicitly in `ARCHITECTURE.md`, not imply authenticity it doesn't have.
- **Enforcement:** `TESTER.md`'s checklist includes a schema-authenticity line: every non-obviously-computed column either cites its vendor-doc source or is marked "internal, not vendor-native." When code and `ARCHITECTURE.md` disagree about a schema (one was fixed, the other wasn't), that's a Rule 1 (No Silent Scope Creep) violation — fix both in the same change, not "fix the one I'm looking at."
- **Rationale (Iteration 3 retro, 2026-08-24):** Three real gaps this iteration (Sprint columns, Custom-field labeling, Xray's Flaky/Automation Coverage %/Test Type) all came from assuming vendor behavior instead of checking it — and checking it, every time, caught a real mistake we'd have shipped. Also caught in this same retro: `ARCHITECTURE.md`'s Xray schema table was never updated after Issue #9's code fix (Flaky/Automation Coverage % dropped in `xray_logs.py`, still listed as columns in the spec) — proof the rule needs an enforcement step, not just good intentions.

### 9. Working Software Is the Default Outcome of Done

- **Rule:** A PBI marked Done must produce a measurable change in the running software's actual behavior — CLI output, generated data, or another user-facing artifact. That's the implicit, default bar, not "tests pass." An exception (e.g., a refactor that changes code without changing meaning) must be stated **explicitly** at Done-Done time, not left to be inferred.
- **Enforcement:** When declaring a PBI Done, state which case applies: (a) behavior changed — point to the artifact/diff that shows it; or (b) deliberately unchanged — say so and show how "unchanged" was verified (e.g., "output reconfirmed byte-identical against yesterday's data"). Silence on this point isn't acceptable — one of the two must be said, every time.
- **Rationale (Iteration 4 retro, 2026-08-25; PO's ruling):** "Working software should be the normal and implicit outcome of a Done PBI." Task 2.1 shipped Done-Done with the CLI's actual output confirmed byte-identical to the prior day — not caught until the PO asked directly what was measurable in working progress. The gap wasn't that groundwork tasks are illegitimate; it's that Task 2.1's "not yet connected to anything" status was never stated as the explicit exception this rule now requires.
- **Reference:** Agile Manifesto, Principle 7 — "Working software is the primary measure of progress."

### 10. Dependent Artifacts Move Together

- **Rule:** When a PBI changes code that implements a domain fact also asserted in a spec document (`ARCHITECTURE.md`'s squad/cluster/schema descriptions, `TESTER.md`'s example values, etc.), the PBI isn't Done until every documented artifact asserting that same fact has been checked and updated to match — in the same PBI, not a follow-up "whenever someone notices." A mismatch found but deliberately not fixed now must be logged as its own explicit backlog item, not left silently inconsistent.
- **Enforcement (a mechanism, not a promise):** Before declaring such a PBI Done, Code greps `specification/*.md` for the domain concept being changed (a squad name, cluster name, field name, duration figure) — not just re-reads the file already being edited. Any hit asserting something no longer true gets fixed in the same commit, or logged as its own bug with a specific file/line reference.
- **Rationale (Iteration 4 retro, 2026-08-25):** `ARCHITECTURE.md`'s Cluster 2 section carried a stale ~30-day timeline for a full Iteration after Task 2.2 implemented the real 5-day duration — the second time this exact failure mode has happened (first: Iteration 3's Xray field drift, Governance Rule 8). Rule 8 only covers vendor-format authenticity; this generalizes the lesson to any documented domain fact, with the same "don't just promise, grep for it" enforcement style as `CLAUDE.md`'s rename-verification rule.
...
| 2026-08-24 | Iteration 3 retro: added Governance Rule 8 (Vendor Format Verification — every schema column needs a cited vendor source or an explicit "internal" label; code and `ARCHITECTURE.md` must agree). Amended Rule 7 (Trunk-Based Development) with two new practices: check for a designated branch before creating one; Code hands the PO a branch-deletion list at the end of each PBI (Code has no delete-branch permission in this environment — deletion is always the PO's action). Documented the independent-RNG-streams design rule in `ARCHITECTURE.md` (root cause of a real PBI 2.1 bug). Documented the rename-verification rule (case-insensitive grep after any repo-wide rename) in `CLAUDE.md`. No blame framing — retro treated both human and AI mistakes as agreement gaps to close, not faults. | Iteration 3 retro, all nine candidate topics answered by Kirschi (PO); items 1–2 and 5–7 converted to working-agreement changes now, items 8–9 explicitly left as work-in-progress (revisit after 2–3 more Iterations) | Kirschi (PO) + PM + Code |
| 2026-08-25 | Iteration 4 retro: added Governance Rule 9 (Working Software Is the Default Outcome of Done — a Done PBI must change the software's actual behavior, or explicitly state the exception) and Rule 10 (Dependent Artifacts Move Together — a grep-the-docs mechanism, not just a promise, when code changes a fact also asserted in `ARCHITECTURE.md`/`TESTER.md`). Amended Rule 7: a PBI cannot be Done while its branch is unmerged — no condition in this project's actual operating model justifies the gap. Considered and explicitly declined a "manually eyeball CSV rows" rule (PO: bugs are normal; the actual bar is whether TESTER.md's automated coverage catches known bug classes, not preventing all future surprises). | Iteration 4 retro, seven candidate topics answered by Kirschi (PO); items 1/2/4/5 converted to Governance Rules now, item 3 explicitly closed with no rule change, item 6 → `TEAM_OPERATING_SYSTEM.md`, item 7 → velocity chart in `session_log.md` | Kirschi (PO) + PM + Code |
