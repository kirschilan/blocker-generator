...
- **Blocker density is window-scoped, not global:** (blockers in week 3 ÷ features in week 3) × 100 = 20–30%. The window is week 3 specifically — where Task 1.3's cluster injection actually concentrates all activity (Auth days 1–3, Checkout/Payments days 2–4) — not the full "weeks 3–5" span from ARCHITECTURE.md's cluster title, which is too broad and dilutes density to ~11%. Global density (all 12 sprints) is expected to be ~1–2% — a single cluster's local footprint, not the percolation-threshold-breaching density that requires Sprint 3's full 3-cluster/8-squad topology.
